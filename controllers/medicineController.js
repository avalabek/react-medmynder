      ...
      .then(patient => {
        ...
        db.Medicine.create(req.body) //then create medicine
          .then(medicine => {
            ...
            res.json(medicine);
            scheduler.scheduler(frequency,medName,instructions,userName,userNumber); //calling the scheduler functionscheduler.scheduler(frequency,medName,instructions,userName,userNumber); //calling the scheduler function
          })
          .catch(err => res.status(422).json(err));
      })
      .catch(err => res.status(422).json(err));
